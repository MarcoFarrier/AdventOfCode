...
public class Day3Sol {
	...
	public static void main(String[] args)
	{
		...
		HashMap<Point, Integer> presentMap = new HashMap<Point, Integer>();
		...
		presentMap.put(new Point(0,0), 2);
		...
		for(int i = 0; i < input.length(); i++)
		{
			...
			if (i % 2 == 0)
			{
				Point roboPoint = new Point(roboSantaX, roboSantaY);
				toAdd = moveSanta(roboPoint, direction);
				roboSantaX = toAdd.x;
				roboSantaY = toAdd.y;
			}
			else {
				Point santaPoint = new Point(santaX, santaY);
				toAdd = moveSanta(santaPoint, direction);
				santaX = toAdd.x;
				santaY = toAdd.y;
			}
			
			int numPresents = 0;
			if(presentMap.containsKey(toAdd))
			{
				numPresents = presentMap.get(toAdd) + 1;
			}
			else numPresents = 1;
			
			presentMap.put(toAdd, numPresents);
			
		}
		
		System.out.println(presentMap.size());
	}
	
	static Point moveSanta(Point current, char direction)
	{
		double x = current.x;
		double y = current.y;
